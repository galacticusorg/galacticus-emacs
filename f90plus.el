...
;; For example:
;; (add-to-list 'auto-mode-alist '("\\.f\\'" . f90plus-mode))

;; Once you have entered f90plus-mode, you can get more info by using
...
;; To customize f90plus-mode for your taste, use, for example:
...
;; 2. Have "f90plus-mode" just recognize F90 syntax, then derived modes
...
(defcustom f90plus-mode-hook nil
...
(defconst f90-keywords-re
  (concat
   ...
   (regexp-opt '("allocatable" "allocate" "assign" "assignment" "backspace"
                 ...
                 "nopass" "pass" "protected" "rank" "same_type_as" "value" "volatile"
                 ;; F2008.
...
\\([ \t]+\\(\\(?:\\sw\\|\\s_\\)+\\)\\)?"
    ...
    '("^[ \t0-9]*\\(\\(\\(?:\\sw\\|\\s_\\)+\\)[ \t]*:[ \t]*\\)?\\(\\(if\\|\
do\\([ \t]*while\\)?\\|select[ \t]*\\(?:case\\|type\\|rank\\)\\|where\\|\
...
logical\\|double[ \t]*precision\\|type[ \t]*(\\(?:\\sw\\|\\s_\\)+)\\|none\\)[ \t]*"
    ...
    ;; F2003 "rank" in a "select rank" block.
    '("\\_<\\(rank[ \t]*\\)[ \t]*(" (1 font-lock-keyword-face t))
    '("\\_<\\(do\\|go[ \t]*to\\)\\_>[ \t]*\\([0-9]+\\)"
...
(defvar f90plus-mode-syntax-table
...
(defconst f90-else-like-re
  "\\(else\\([ \t]*if\\|where\\)?\\|case[ \t]*\\(default\\|(\\)\\|rank[ \t]*\\(default\\|(\\)\\|\
\\(class\\|type\\)[ \t]*is[ \t]*(\\|class[ \t]*default\\)"
  "Regexp matching an ELSE IF, ELSEWHERE, CASE, RANK, CLASS/TYPE IS statement.")
...
(add-to-list 'hs-special-modes-alist
             `(f90plus-mode ,f90-start-block-re ,f90-end-block-re
...
(define-abbrev-table 'f90plus-mode-abbrev-table
...

;;;###autoload
(define-derived-mode f90plus-mode prog-mode "F90"
...
Turning on F90 mode calls the value of the variable `f90plus-mode-hook'
with no args, if that value is non-nil."
  :group 'f90
  :abbrev-table f90plus-mode-abbrev-table
...
\\|select[ \t]*\\(case\\|type\\|rank\\)\\|case\\|rank\\|where\\|forall\\|\
block\\|critical\\)\\_>")
...
(defun f90-prepare-abbrev-list-buffer ()
  ...
  (with-current-buffer (get-buffer-create "*Abbrevs*")
    (erase-buffer)
    (insert-abbrev-table-description 'f90plus-mode-abbrev-table t)
    (goto-char (point-min))
...
escape character."
  (or (derived-mode-p 'f90plus-mode)
